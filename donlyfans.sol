...
contract dOnlyFans is PullPayment {
    ...
    function createProfile(uint256 price) public returns(bool) {
        if ((creators[msg.sender].CCaddress)== msg.sender){ // the creator profile already exists
            return false;
        }
    ...
    function getSuscribers(address creatorAddress) public view returns(address[] memory) {
    ...
    // function unsuscribe(address creatorAddress){
